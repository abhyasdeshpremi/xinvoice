...
class Purchase extends CI_Controller {
    ...
    public function createinvoiceID(){
        $invoice = $this->Invoice_model->invoice_id("purchase");
        if(count($invoice) == 0){
           $invoiceNewID = invoiceIDCreation();
           $newInvoiceResult = $this->Invoice_model->invoiceInitial($invoiceNewID, "purchase");
           if($newInvoiceResult){
                redirect('createpurchaseinvoice/'.$invoiceNewID); 
           }else{
            ...
            redirect('createpurchaseinvoice/'.$invoice[0]->unique_invioce_code); 
        }
    }
    ...
    public function createinvoice($id = null){
        ...
        $data['clients'] = $this->Invoice_model->client_list_ONLY_supllier();
    ...
    public function viewInvoicePDF($id = null, $mode = "landscape", $download = false){
        ...
        $data['link'] = 'createpurchaseinvoicepdf/'.$id.'/'.$mode.'/'.$download;
        $this->template->set('title', 'View Purchase Invoice PDF');
    ...
    public function saveItemInInvoice(){
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            if($invoice_item_save_result['code']){
                /***
                 * buy:- Adjust item stock when purchase item
                 */
                $data['item_code'] = $data['itemcode'];
                $data['item_name'] = $data['itemname'];
                $data['stocktype'] = "buy";
                $data['stockunit'] = $data['quatity'];
                $data['stockcomment'] = "Added items stock when purchase invoice generated";
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            if($invoice_item_save_result['code']){
                ...
                if ($isAction) {
                    ...
                    $data['stockcomment'] = "Updated items stock when purchase invoice generated";
                    $stock_item_save_result = $this->Stock_model->saveStock($data);
                }
    ...
    public function invoicedetails(){
        ...
        $this->template->load('default_layout', 'contents' , 'purchase/purchaseinvoicedetail', $data);
    }
